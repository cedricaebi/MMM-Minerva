Module.register('MMM-Minerva', {
	defaults: {
		...
		dataset: 'modules/MMM-Minerva/dataset/',
		...
		debug: 0,
		// Animation fadeout speed for the compliment text
		fadeSpeed: 2000,
		// How often the compliment text refreshes
		updateInterval: 3000
	},
	...
	login_user: function (name) {
		...
		if (name === 'unknown') {
			...
			this.personName = this.translate('stranger')
		} else {
	...
	logout_user: function (name) {
		...
		if (this.users.includes(name)) {
			...
			this.config.debug && Log.log(this.userClasses)
			let oldUserClasses = Object.assign({}, this.userClasses) // assign works doing a shallow copy because the object is simply enough
			...
			if (this.users.length === 0) {
				...
				if (name === 'unknown') {
					...
					// we want to show the new classes allowed by this target state (noface state)
					// copy the config classes to a new array
					let oldClasses = this.config.classes_known.slice()
					this.config.debug && Log.log('Adding ' + name + ' to classlist: ' + this.config.classes_known)
				...
				let shownClasses = this.get_class_set(oldUserClasses)
				...
				let remainingClasses = this.get_class_set(this.userClasses)
	...
	socketNotificationReceived: function (notification, payload) {
		...
		if (payload.action === 'emotion') {
			this.config.debug && Log.log('New emotion received ' + payload.emotion)
	...
	getDom: function () {
		...
		if (this.personName !== null) {
			responseText = this.translate(this.emotion).replace('%person', this.personName)
		} else {
			responseText = ''
		}
		...
		let compliment = document.createElement('span')
